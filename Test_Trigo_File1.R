...
cc <- function(lambda) lambda * digamma(lambda)
d <- function(lambda) lambda * trigamma(lambda)
M <- function(lambda) cbind(c(
  - h8(lambda) - (1 / (d(lambda) - 1)) * (h10(lambda)  +  h6(lambda, lambda + 1, 1) * (cc(lambda) - d(lambda) * (cc(lambda) + 1))) , 
  - h9(lambda) - (1 / (d(lambda) - 1)) * (h11(lambda)  +  h7(lambda, lambda + 1, 1) * (cc(lambda) - d(lambda) * (cc(lambda) + 1)))  
))
Sigma <- function(lambda){
  matrix(c(
     0.5 - (lambda / (d(lambda) - 1)) * (d(lambda) * h6(lambda, lambda + 1, 1)^2 + h10(lambda)^2 -
                                      2 * h6(lambda, lambda + 1, 1) * h10(lambda)),
     (lambda / (d(lambda) - 1)) * (h6(lambda, lambda + 1, 1) * (h11(lambda) - d(lambda) * h7(lambda, lambda + 1, 1)) +
                                     h10(lambda) * (h7(lambda, lambda + 1, 1) - h11(lambda))),
     (lambda / (d(lambda) - 1)) * (h6(lambda, lambda + 1, 1) * (h11(lambda) - d(lambda) * h7(lambda, lambda + 1, 1)) +
                                     h10(lambda) * (h7(lambda, lambda + 1, 1) - h11(lambda))),
     0.5 - (lambda / (d(lambda) - 1)) * (d(lambda) * h7(lambda, lambda + 1, 1)^2 + h11(lambda)^2 -
                                      2 * h7(lambda, lambda + 1, 1) * h11(lambda))
  ), ncol  = 2)
}
Sigma.inv <- function(lambda) solve(Sigma(lambda))
V <- function(lambda) t(M(lambda)) %*%  Sigma.inv(lambda) %*% M(lambda)
...
Sigma.inv(lambda)
solve(diag(2)/2 - G(lambda, beta) %*% solve(R(lambda, beta)) %*% t(G(lambda, beta)))

## ---- power curve for lambda = 0.5
lambda <- .5; beta <- 1
ncp <- diag(t(delta) %*% V(lambda) %*% delta) # non-centrality parameter (ncp) of the noncentral chi-square distribution 
...
Sigma <- matrix(c(
  0.5 - (h6(1,2,1)) ^ 2 - (6 / pi ^ 2) * ((euler - 1) * h6(1,2,1) + h8(1)) ^ 2,
  -h6(1,2,1) * h7(1,2,1) - (6 / pi^2) * ((euler - 1) * h6(1,2,1) + h8(1)) * ((euler - 1) * h7(1,2,1) + h9(1)),
  -h6(1,2,1) * h7(1,2,1) - (6 / pi^2) * ((euler - 1) * h6(1,2,1) + h8(1)) * ((euler - 1) * h7(1,2,1) + h9(1)),
  0.5 - (h7(1,2,1)) ^ 2 - (6 / pi ^ 2) * ((euler - 1) * h7(1,2,1) + h9(1)) ^ 2),
  ncol = 2)
...
solve(diag(2)/2 - G(beta, rho) %*% solve(R(beta, rho)) %*% t(G(beta, rho)))
Sigma.inv

## ---- power curve 
ncp <- diag(t(delta) %*% V %*% delta) # non-centrality parameter (ncp) of the noncentral chi-square distribution 
power <- pchisq(critical.value, df = 2, ncp = ncp, lower.tail = FALSE)
...
Sigma.ML <- function(lambda) diag(c(1 / 2 - h1(lambda) ^ 2 / lambda, 
                                    1 / 2 - h2(lambda) ^ 2 / (gamma(1 / lambda) * gamma(2 - 1 / lambda))))
Sigma.inv.ML <- function(lambda) solve(Sigma.ML(lambda))
d <- function(lambda) h2(lambda) / (lambda ^ (1 / lambda - 1) * gamma(1 / lambda))
Sigma.MM <- function(lambda) diag(c(
  1 / 2 - h1(lambda)  *  h4(lambda) + h1(lambda) ^ 2 / (4 * C3(lambda)),
  1 / 2 -  d(lambda) / C2(lambda) * (
    2  * h5(lambda) * gamma(2 / lambda) / lambda ^ (1 / lambda) / gamma(3 / lambda) - d(lambda))))
Sigma.inv.MM <- function(lambda) solve(Sigma.MM(lambda))

V.ML <- function(lambda) t(M.ML(lambda)) %*%  Sigma.inv.ML(lambda) %*% M.ML(lambda)
V.MM <- function(lambda) t(M.MM(lambda)) %*%  Sigma.inv.MM(lambda) %*% M.MM(lambda)
level <- 0.05 # the nominal significance level is set to 0.05 
# We calculate the critical value for a test with level of 0.05, defined as the 5% quantile (in the right tail) of a chi-square distribution with 2 degrees of freedom
critical.value <- qchisq(level, df = 2, lower = FALSE) 
lambda <- 1.5
...
Sigma.inv.ML(lambda)
solve(diag(2)/2 - G(lambda, sigma) %*% solve(R.ML(lambda, sigma)) %*% t(G(lambda, sigma)))

## ---- Validation of Sigma.inv.MM
EPD.test.MM(rnorm(10), lambda)$Sigma.inv
Sigma.inv.MM(lambda)
solve(diag(2)/2 - G(lambda, sigma) %*% solve(R.MM(lambda, sigma))  %*% t(J(lambda, sigma)) -  J(lambda, sigma) %*% solve(R.MM(lambda, sigma))  %*% t(G(lambda, sigma)) + G(lambda, sigma) %*% solve(R.MM(lambda, sigma))  %*% t(G(lambda, sigma)))
...
## ---- power curves 
ncp.ML <- diag(t(delta) %*% V.ML(lambda) %*% delta) # non-centrality parameter (ncp) of the noncentral chi-square distribution 
ncp.MM <- diag(t(delta) %*% V.MM(lambda) %*% delta) # non-centrality parameter (ncp) of the noncentral chi-square distribution 
