# This file contains the R functions for the article:
# A new omnibus goodness-of-fit test based on trigonometric moments
...
#### APD distribution (see section local alternative), lambda > 0, 0 < alpha < 1, rho > 0 
...
EPD.test.MM <- function(x, lambda, mu = "unknown", sigma = "unknown"){
  ...
  MM.mu.sigma <- MM.EPD(x, lambda, mu, sigma)
...
######## exponential-generalized gamma or exp-GG distribution
...
Student.test.MM <- function(x, lambda, mu = "unknown", sigma = "unknown"){
  ...
  MM.mu.sigma <- MM.Student(x, lambda, mu, sigma)
  mu.hat <- MM.mu.sigma[1]
  sigma.hat <- MM.mu.sigma[2]
  Fi <- pt((x - mu.hat) / sigma.hat, lambda)
  neg2L <- - 2 * sum(log(dStudent(x, lambda = lambda, mu = mu.hat, sigma = sigma.hat)))
...
logEPD.test.MM <- function(x, lambda, mu = "unknown", sigma = "unknown"){
  ...
  MM.mu.sigma <- MM.logEPD(x, lambda, mu, sigma)
  mu.hat <- MM.mu.sigma[1]
  sigma.hat <- MM.mu.sigma[2]
  Fi <- plogEPD(x, lambda = lambda, mu = mu.hat, sigma = sigma.hat)
  neg2L <- - 2 * sum(log(dlogEPD(x, lambda = lambda, mu = mu.hat, sigma = sigma.hat)))
...
#### half-EPD distribution, lambda >= 1
...
halfEPD.test.MM <- function(x, lambda, sigma = "unknown"){
  ...
  sigma.hat <- MM.halfEPD(x, lambda, sigma) 
  Fi <- phalfEPD(x, lambda = lambda, sigma = sigma.hat)
  neg2L <- - 2 * sum(log(dhalfEPD(x, lambda = lambda, sigma = sigma.hat)))
  G <- 1 / sigma.hat * matrix(c(h6(1 / lambda, 1 / lambda + 1, 1), h7(1 / lambda, 1 / lambda + 1, 1)), nrow = 2) 
  R <- C2 / sigma.hat ^ 2
  ...
  rownames(Z.tau.bar) = c("Z(Cn)", "Z(Sn)")
  p.value <- pchisq(Tn, df = 2, lower.tail = FALSE)
  return(list(MM.sigma = sigma.hat, Sigma.inv = Sigma.inv, neg2L = neg2L, tau.bar.sqrtn = tau.bar.sqrtn, Z.tau.bar = Z.tau.bar, Tn = Tn, p.value = p.value))
}
...
Weibull.test.ML <- function(x, beta = "unknown", rho = "unknown"){
  ...
  neg2L <- - 2 * sum(log(dWeibull(x, beta = beta.hat, rho = tho.hat)))
...
dNakagami <- function(x, alpha, omega) 2 / gamma(alpha) * (alpha / omega) ^ alpha * x ^ (2 * alpha - 1) * exp(- alpha / omega * x ^ 2)
pNakagami <- function(x, alpha, omega) pgamma(alpha / omega * x ^ 2 , shape = alpha, scale = 1)
qNakagami <- function(p, alpha, omega) sqrt(omega / alpha * qgamma(p, shape = alpha, scale = 1)) 
rNakagami <- function(n, alpha, omega) sqrt(omega / alpha * rgamma(n, shape = alpha, scale = 1)) 
ML.Nakagami <- function(x, alpha = "unknown", omega = "unknown") {
  if (alpha == "unknown" & omega == "unknown") {
    omega.ML <- mean(x ^ 2)
    mlx2 <- mean(log(x ^ 2))
    alpha.MM <- omega.ML ^ 2 / mean((x ^ 2 - omega.ML) ^ 2)
    g.alpha <- function(alpha)  digamma(alpha) - log(alpha) + log(omega.ML)  - mlx2
    alpha.ML <- uniroot(g.alpha, interval = c(alpha.MM / 5, alpha.MM * 5), tol = 1e-09)$root
  }
  if (alpha == "unknown" & omega != "unknown") {
    mx2 <- mean(x ^ 2)
    mlx2 <- mean(log(x ^ 2))
    alpha0 <- omega ^ 2 / mean((x ^ 2 - omega) ^ 2)
    g.alpha <- function(alpha)   digamma(alpha) - log(alpha)  + log(omega) - mlx2 + mx2 / omega  - 1
    alpha.ML <- uniroot(g.alpha, interval = c(alpha0 / 5, alpha0 * 5), tol = 1e-09)$root
    omega.ML <- omega
  }
  if (alpha != "unknown" & omega == "unknown") {
    omega.ML <- mean(x ^ 2)
  ...
  return(c(alpha.ML, omega.ML))
}
Nakagami.test.ML <- function(x, alpha = "unknown", omega = "unknown"){
  n <- length(x)
  ML.alpha.omega <-  ML.Nakagami(x, alpha, omega)
  alpha.hat <- ML.alpha.omega[1]
  omega.hat <- ML.alpha.omega[2]
  Fi <- pgamma(alpha.hat / omega.hat * x ^ 2 , shape = alpha.hat, scale = 1)
  neg2L <- - 2 * sum(log(dNakagami(x, alpha = alpha.hat, omega = omega.hat)))
  h4.alpha.hat <- h6(alpha.hat, alpha.hat + 1, 1)
  h5.alpha.hat <- h7(alpha.hat, alpha.hat + 1, 1)
  G <- matrix(c(h10(alpha.hat) - h4.alpha.hat, h11(alpha.hat) - h5.alpha.hat, alpha.hat / omega.hat * h4.alpha.hat, alpha.hat / omega.hat * h5.alpha.hat), ncol = 2)
  R <- matrix(c(trigamma(alpha.hat) - 1 / alpha.hat, 0, 0, alpha.hat / omega.hat ^ 2), ncol = 2)
  U <- (1:2)[c(alpha == "unknown", omega == "unknown")] 
  G <- as.matrix(G[,U]); R <- R[U,U]
  if (length(U) > 0)  Sigma <- diag(2)/2 - G %*% solve(R) %*% t(G) else Sigma <- diag(2)/2
  Sigma.inv <- solve(Sigma)  
  tau.bar.sqrtn <- sqrt(n) * matrix(c(mean(cos(2 * pi * Fi)), mean(sin(2 * pi * Fi))), ncol = 1)
  ...
  rownames(Z.tau.bar) = c("Z(Cn)", "Z(Sn)")
  p.value <- pchisq(Tn, df = 2, lower.tail = FALSE)
  return(list(ML.alpha.omega = ML.alpha.omega, Sigma.inv = Sigma.inv, neg2L = neg2L, tau.bar.sqrtn = tau.bar.sqrtn, Z.tau.bar = Z.tau.bar, Tn = Tn, p.value = p.value))
}
